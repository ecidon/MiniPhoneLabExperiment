 ...
 * <ul>
 * <li>
 * The user presses the "Picture" button, resulting in a call to {@link #takePicture(String file)}.
 * </li>
 * <li>
 * {@link #takePicture(String file)} initiates a pre-capture sequence that triggers the camera's built-in
...
public class Camera2RawFragment extends Fragment
    ...
    private String SERVER_ADDRESS = "http://192.168.1.180:8080/";
     ...
     */
    private static final long PRECAPTURE_TIMEOUT_MS = Long.MAX_VALUE;
    ...
    private final CameraCaptureSession.CaptureCallback mCaptureCallback
        ...
        public void onCaptureStarted(CameraCaptureSession session, CaptureRequest request,
            ...
            String file = imageList.get(imageIdx).replace("\"", "");
            if (jpegBuilder != null) jpegBuilder.setFile(file);
            if (rawBuilder != null) rawBuilder.setFile(file);
            if (jpegBuilder != null) jpegBuilder.setServer(SERVER_ADDRESS);
            if (rawBuilder != null) rawBuilder.setServer(SERVER_ADDRESS);
        }
    ...
    class RequestImageListTask extends AsyncTask<Void,Void,Void> {
        protected Void doInBackground(Void... params) {
            Log.i(TAG, "Requesting image list");
            try {
                URL url = new URL(String.format("%slist", SERVER_ADDRESS));
                HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
                urlConnection.setRequestMethod("GET");
                try {
                    int code = urlConnection.getResponseCode();
                    Log.i(TAG, String.format("response code: %d", code));
                    ...
                    Log.i(TAG, String.format("list: %s", jsonString));
                    imageList = Arrays.asList(jsonString
                            .replace("[", "")
                            .replace("]", "")
                            .split("\\s*,\\s*"));
                } finally {
    ...
    private void runExperiment() {
        ...
        Log.i(TAG, String.format("index is %d", imageIdx));
        String file = imageList.get(imageIdx).replace("\"","");
        Log.i(TAG, String.format("next file: %s", file));
        takePicture(file);
    }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.picture: {
                try {
                    new RequestImageListTask().execute().get();
                } catch (InterruptedException e) {
                    e.printStackTrace();
                } catch (ExecutionException e) {
                    e.printStackTrace();
                }
                Log.i(TAG, Arrays.deepToString(imageList.toArray()));
                runExperiment();
                break;
            }
            case R.id.info: {
    ...
    class requestImageDisplayTask extends AsyncTask<String,Void,Void> {
        protected Void doInBackground(String... file) {
            Log.i(TAG, String.format("Requesting display image %s", file[0]));
            try {
                URL url = new URL(String.format("%s%s", SERVER_ADDRESS, file[0]));
                HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
                urlConnection.setRequestMethod("GET");
                try {
                ...
                e.printStackTrace();
                Log.e(TAG, "Error getting image display");
            }
            return null;
        }
    }
     ...
     */
    private void takePicture(String file) {

        try {
            new requestImageDisplayTask().execute(file).get();
        } catch (InterruptedException e) {
            e.printStackTrace();
        } catch (ExecutionException e) {
            e.printStackTrace();
    ...
    private static class ImageSaver implements Runnable {
        ...
        private ImageSaver(Image image, String file, CaptureResult result, String SERVER_ADDRESS,
            ...
            mContext = context;
            mReader = reader;
            mSERVER_ADDRESS = SERVER_ADDRESS;
        }

        class PostJpegTask extends AsyncTask<byte[],Void,Void> {
            @Override
        ...
        public void run() {
            ...
            switch (format) {
                ...
                case ImageFormat.RAW_SENSOR: {
                    ...
                    try {
                        dngCreator.writeImage(byteOutput, mImage);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
        ...
        public static class ImageSaverBuilder {
            ...
            public synchronized ImageSaverBuilder setServer(final String SERVER_ADDRESS) {
                mSERVER_ADDRESS = SERVER_ADDRESS;
                return this;
            }
